...
export default function AuthForm(props: AuthformPros) {
  ...
  const [resetPwdToken, setResetPwdToken] = React.useState<string | null>(
  ...
  useEffect(() => {
    switch (props.formContext) {
      ...
      case TextLinkHrefEnum.resetPwd:
        ...
        if (resetPwdToken) setIsLoading(false);
        else alert('Reset token is not present');
        break;
      default:
        setSubmitBtnText('Go');
        setIsLoading(false);
  ...
  const submitForgotPassword = async () => {
    try {
      ...
      alert(
        'Vous allez recevoir un email contenant le lien pour modifier votre mot de passe',
      );
      router.push('/login');
    } catch (error: any) {
      alert(
  ...
  const submitResetPassword = async () => {
    try {
      ...
      alert(
        'Erreur lors de la modification de votre mot de passe : ' +
          error.message,
      );
    }
  };
